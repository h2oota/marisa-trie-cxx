...
unsigned char key_get(const marisa::Key* key, std::size_t i, const struct exception_record ** exception)
{
     *exception = nullptr;
     try {
	  return (unsigned char)(*key)[i];
...
const unsigned char *key_ptr(const marisa::Key* key, const struct exception_record ** exception)
{
     *exception = nullptr;
     try {
	  return (const unsigned char *)key->ptr();
     } catch (const std::exception &ex) {
	  *exception = save_exception(ex);
     }
     return nullptr;
